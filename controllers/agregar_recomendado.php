...
try {
    ...
    // 2. Verificar si ya está en recomendados
    $stmtCheckRec = $con->prepare("SELECT id FROM recomendados WHERE noticia_id = ?");
    $stmtCheckRec->bind_param("i", $noticia_id);
    $stmtCheckRec->execute();
    if ($stmtCheckRec->get_result()->num_rows > 0) {
        echo json_encode(['success' => false, 'error' => 'La noticia ya está en la lista de recomendados']);
        exit;
    }
    ...
    if (intval($countRow['total']) >= 10) {
        echo json_encode(['success' => false, 'error' => 'Se ha alcanzado el límite máximo de 10 recomendaciones']);
        exit;
    }

    // 4. Obtener orden máximo actual
    $ordenRes = $con->query("SELECT COALESCE(MAX(orden), 0) AS max_orden FROM recomendados");
