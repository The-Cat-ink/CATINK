...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4" style="flex-wrap: wrap; gap: 12px;">
        <h1 style="margin:0;">Lo más Recomendado</h1>
    </div>
    ...
    <div class="stats-grid mb-4">
        <div class="stat-card">
            ...
            <div class="stat-info">
                <span class="stat-value" id="countBadge"><?= $totalRecomendados ?> / 10</span>
                <span class="stat-label">Artículos Recomendados</span>
            </div>
        </div>
    </div>

    <div class="row">
        <!-- Columna Izquierda: Lista de recomendados (Drag & Drop) -->
            ...
            <div class="card shadow-sm" style="background: var(--card-bg); border: 1px solid var(--border); border-radius: 12px; height: 100%;">
                <div class="card-header" style="background: transparent; border-bottom: 1px solid var(--border); padding: 16px;">
                    <h3 class="card-title" style="margin: 0; font-size: 1.1rem; font-weight: 700; color: var(--text);">
                        <i class="bi bi-grip-vertical" style="color: var(--muted); margin-right: 4px;"></i> Lista de Recomendados (Arrastra para ordenar)
                    </h3>
                </div>
                <div class="card-body" style="padding: 16px;">
                    <div id="recomendadosList" class="d-flex flex-column gap-3">
                        <?php if (empty($recomendados)): ?>
                            <div id="emptyRecomendados" class="text-center py-4" style="color: var(--muted); font-style: italic;">
                                No hay artículos recomendados. Agrega noticias desde el buscador de la derecha.
                            </div>
                        <?php else: ?>
                            <?php foreach ($recomendados as $index => $rec): ?>
                                <div class="recomendado-item d-flex align-items-center gap-3 p-3" data-id="<?= $rec['id'] ?>" style="background: var(--bg); border: 1px solid var(--border); border-radius: 8px; cursor: grab;">
                                    <?php if ($ACL['editar']): ?>
                                    ...
                                    <div style="width: 80px; aspect-ratio: 16/9; border-radius: 4px; overflow: hidden; background: var(--border); flex-shrink: 0;">
                                        <img src="<?= imageUrl($rec['crop3']) ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                    </div>
                                    ...
                                    <?php if ($ACL['editar']): ?>
                                        <button class="btn btn-delete btn-quitar" data-id="<?= $rec['id'] ?>" style="padding: 6px 10px; font-size: 0.82rem; border-radius: 6px; border: 1px solid #e53e3e; background: transparent; color: #e53e3e;" title="Quitar de recomendados">
...
document.addEventListener('DOMContentLoaded', () => {
    const recomendadosList = document.getElementById('recomendadosList');
    ...
    function renumerarRankings() {
        const items = recomendadosList.querySelectorAll('.recomendado-item');
    ...
    function obtenerConteo() {
        return recomendadosList.querySelectorAll('.recomendado-item').length;
    }
    ...
    function guardarOrdenRecomendados() {
        const items = recomendadosList.querySelectorAll('.recomendado-item');
        const orden = [];
        items.forEach((item, index) => {
            orden.push({
        ...
        fetch('./../controllers/reordenar_recomendados.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ recomendados: orden })
        })
        .then(r => r.json())
        .then(d => {
            if (!d.success) {
                console.error('Error al guardar el orden:', d.error);
                alert('No se pudo guardar el orden de las recomendaciones');
            }
        })
        .catch(err => {
    ...
    // 6. Eliminar artículo recomendado
    recomendadosList.addEventListener('click', e => {
        ...
        fetch('./../controllers/eliminar_recomendado.php', {
            method: 'POST',
            body: form
        })
        .then(r => r.json())
        .then(d => {
            if (d.success) {
                const item = btn.closest('.recomendado-item');
                item.remove();
                actualizarContador();
                renumerarRankings();
                ...
                alert(d.error || 'Error al eliminar recomendación');
            }
        })
    ...
    function buscarNoticias(query) {
        fetch(`./../controllers/buscar_noticias_admin.php?q=${encodeURIComponent(query)}`)
        .then(r => r.json())
        .then(d => {
            if (d.success) {
    ...
    function renderizarResultados(noticias) {
        ...
        evaluarLimiteRecomendaciones();
    }

    function evaluarLimiteRecomendaciones() {
        const total = obtenerConteo();
        const botones = buscarResultados.querySelectorAll('.btn-agregar');
        botones.forEach(btn => {
            if (total >= 10) {
                btn.disabled = true;
                btn.title = 'Límite máximo de 10 recomendaciones alcanzado';
    ...
    // 8. Agregar noticia a recomendados
    buscarResultados.addEventListener('click', e => {
        ...
        const total = obtenerConteo();
        if (total >= 10) {
            alert('Límite máximo de 10 recomendaciones alcanzado');
            return;
        }
        ...
        fetch('./../controllers/agregar_recomendado.php', {
            method: 'POST',
            body: form
        })
        .then(r => r.json())
        .then(d => {
            if (d.success) {
                ...
                const empty = document.getElementById('emptyRecomendados');
                if (empty) empty.remove();

                // Crear elemento recomendado en el DOM
                const nuevoIndex = obtenerConteo() + 1;
                const div = document.createElement('div');
                div.className = 'recomendado-item d-flex align-items-center gap-3 p-3';
                div.dataset.id = d.id;
                ...
                recomendadosList.appendChild(div);
                actualizarContador();
                ...
                alert(d.error || 'Error al agregar recomendación');
            }
        })
        .catch(err => console.error(err));
    });

    // 9. Agregar recomendación personalizada con recorte de imagen
    ...
    if (customItemForm) {
        customItemForm.addEventListener('submit', e => {
            ...
            fetch('./../controllers/agregar_recomendado_personalizado.php', {
                method: 'POST',
                body: formData
            })
            .then(r => r.json())
            .then(d => {
                if (d.success) {
                    ...
                    alert(d.error || 'Error al agregar recomendación personalizada');
                }
            })
            .catch(err => {
                console.error(err);
